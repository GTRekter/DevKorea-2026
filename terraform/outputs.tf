...
output "linkerd_link_instructions" {
  ...
  value       = <<-EOT
    ...
    # Generate kubeconfig for ${cluster_name}
    ...
    kubectl config rename-context "${cluster_name}" "${cluster_name}" --kubeconfig=./kubeconfigs/kubeconfig-${cluster_name}.yaml
    ...
    # Verify contexts are available (should show friendly names like devkorea-nks-1, devkorea-nks-2)
    ...
    # This allows ${pair.target} to discover and route to services in ${pair.source}
